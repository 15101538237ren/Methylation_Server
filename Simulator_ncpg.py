...
def load_ncpg_d_from(nrange=range(1,10,2),drange=range(1,10,2),dirpath=""):
    ...
    for idx,n in enumerate(nrange):
        ...
        if idx ==0:
            tmp_arr=[]
            for d2 in drange:
                tmp_arr.append(str(d2))
            line_2_wrt=",".join(tmp_arr)
            out_file.write("0,"+line_2_wrt+"\n")
        for d in drange:
            gen_mean[n][d]=gen_sum[n][d]/len_hash
            line_arr.append(str(gen_mean[n][d]))
        line_2_wrt=",".join(line_arr)
        out_file.write(str(n)+","+line_2_wrt+"\n")
    out_file.close()
    return nd_hash
if __name__ == '__main__':
    ...
    reaction_param_file_path = param_base_path+"E135_reaction.txt"
    reaction_param_file_for_0_path = param_base_path+"E135_reaction.txt"
    param_hash = load_param_from_file(procedure_param_file_path)

    ncpgs=range(2,11,1)
    ncpgs.extend(range(11,20,2))
    ncpgs.extend(range(20,100,5))
    ncpgs.extend(range(100,1001,100))
    # for ncpg in ncpgs:
    #     print str(ncpg)+",",
    distances=[2,10,20,30,50,100,150,200,300,500]
    out_dirs=["ncpg_E135_u_90_m002"]#,"ncpg_u0_m1"]
    for out_dir in out_dirs:
        nd_hash=load_ncpg_d_from(nrange=ncpgs,drange=distances,dirpath=out_dir)
    # out_dir="ncpg_E135_u_0_m100"
    # for now_distance in distances:
    #     for ncpg in ncpgs:
    #         param_hash["OUTPUT_DIR_first"]=out_dir+os.sep+"d"+str(now_distance)+os.sep+"n_"+str(ncpg)+"_d_"+str(now_distance)
    #         param_hash["max_cpg_site_param"]=str(ncpg)
    #         param_hash["just_simulate"]="True"
    #         start_simulation(function_util,reaction_param_file_path,reaction_param_file_for_0_path,now_distance,**param_hash)
